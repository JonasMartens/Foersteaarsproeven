    ...
    class SQLRepository : IRepository
    {
        ...
        public void InsertSmsKontakter(int smsID, List<Kunde> kontaktIDListe)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    Kunde kontaktID = kontaktIDListe[i];

                    using (SqlCommand cmd = new SqlCommand("INSERT INTO KontaktSmsSammenlægning (SmsID, KontaktID) VALUES (@SmsID, @KontaktID)", connection))
                    {
                        cmd.Parameters.AddWithValue("@SmsID", 7);
                        cmd.Parameters.AddWithValue("@KontaktID", kontaktID.ID);
